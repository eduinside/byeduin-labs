/* ================================================================
   world-landmarks — 조사 보고서 모드 데이터 (27개국)
   대륙별 배분: 아시아 8 · 유럽 6 · 아프리카 4 · 북아메리카 3 · 남아메리카 4 · 오세아니아 2
...
window.WL_COUNTRIES = [
  ...
  {
    id: 'tr', nameKo: '튀르키예', continent: 'asia',
    lat: 39.9, lng: 32.9,
    flag: { meaning: '붉은 바탕에 하얀 초승달과 별이 그려져 있어요. 초승달과 별은 이 나라에서 오래 쓰여 온 상징이에요.' },
    location: { desc: '아시아와 유럽 두 대륙에 걸쳐 있는 나라예요. 좁은 바닷길(보스포루스 해협)이 두 대륙을 나눠요.' },
    capital: { name: '앙카라', desc: '이스탄불이 아니라 앙카라가 수도예요. 나라 한가운데 자리 잡은 도시랍니다.' },
    greeting: { text: '메르하바 (Merhaba)', korean: '메르하바' },
    foods: [
      { name: '케밥', emoji: '🍢', desc: '고기를 꼬치에 꿰거나 얇게 저며 구워 빵·채소와 함께 먹는 음식이에요.' },
      { name: '바클라바', emoji: '🍯', desc: '얇은 반죽을 겹겹이 쌓고 견과류와 시럽을 넣은 달콤한 후식이에요.' }
    ],
    prides: [
      { name: '소피아 성당(하기아 소피아)', emoji: '🕌', desc: '천 년이 넘도록 성당과 사원으로 쓰인, 커다란 돔 지붕의 건축물이에요.' },
      { name: '카파도키아', emoji: '🎈', desc: '바위가 버섯 모양으로 솟은 신비로운 땅으로, 열기구를 타고 하늘에서 내려다봐요.' }
    ],
    cultures: [
      { name: '나자르 본주', emoji: '🧿', desc: '푸른 눈 모양의 장식으로, 나쁜 일을 막아 준다고 여겨 몸이나 집에 지녀요.' },
      { name: '튀르키예 차', emoji: '🫖', desc: '작은 유리잔에 홍차를 담아 손님과 나누는 문화가 있어요.' }
    ]
  },
  ...
  },
  {
    ...
    prides: [
      { name: '성 바실리 성당', emoji: '⛪', desc: '알록달록한 양파 모양 지붕이 여러 개 솟아 있는 붉은 광장의 성당이에요.' },
  ...
  {
    id: 'tz', nameKo: '탄자니아', continent: 'africa',
    lat: -6.2, lng: 35.7,
    flag: { meaning: '초록·노랑·검정·파랑이 대각선으로 어우러진 국기로, 땅·자원·사람·바다를 뜻해요.' },
    location: { desc: '아프리카 동쪽에서 인도양과 맞닿아 있는 나라예요. 아프리카에서 가장 높은 산인 킬리만자로가 있어요.' },
    capital: { name: '도도마', desc: '나라 한가운데 자리 잡은 수도예요. 바닷가의 큰 도시 다르에스살람과 함께 중심 역할을 해요.' },
    greeting: { text: '잠보 (Jambo)', korean: '잠보' },
    foods: [
      { name: '우갈리', emoji: '🌽', desc: '옥수수 가루를 되직하게 반죽한 주식으로, 반찬과 곁들여 먹어요.' },
      { name: '필라우', emoji: '🍚', desc: '향신료로 향을 낸 쌀밥 요리로, 특별한 날에 즐겨 먹어요.' }
    ],
    prides: [
      { name: '세렝게티 초원', emoji: '🦓', desc: '수많은 야생동물이 무리 지어 이동하는, 드넓게 펼쳐진 초원이에요.' },
      { name: '킬리만자로산', emoji: '🏔️', desc: '적도 근처인데도 꼭대기에 눈이 덮여 있는, 아프리카에서 가장 높은 산이에요.' }
    ],
    cultures: [
      { name: '마사이족의 춤', emoji: '💃', desc: '제자리에서 높이 뛰어오르며 추는 전통 춤으로 알려진 사람들이 살아요.' },
      { name: '캉가', emoji: '🧵', desc: '밝은 무늬와 좋은 글귀를 넣은 천을 몸에 둘러 입는 전통이 있어요.' }
    ]
  },
  ...
  },
  {
    ...
    prides: [
      ...
      { name: '토템 조각상', emoji: '🗿', desc: '동물이나 조상의 얼굴을 기둥에 새겨 세운, 원주민의 전통 조각이에요.' }
    ],
    cultures: [
  ...
  {
    id: 'cl', nameKo: '칠레', continent: 'samerica',
    lat: -33.5, lng: -70.7,
    flag: { meaning: '하양·빨강 바탕에 파란 네모와 하얀 별 하나가 그려져 있어요. 별은 나라를 밝히는 길잡이를 뜻해요.' },
    location: { desc: '남아메리카 서쪽 바닷가를 따라 남북으로 아주 길게 뻗은 나라예요. 안데스산맥과 태평양 사이에 있어요.' },
    capital: { name: '산티아고', desc: '안데스산맥이 병풍처럼 둘러싼 칠레의 수도예요.' },
    greeting: { text: '올라 (Hola)', korean: '올라' },
    foods: [
      { name: '파스텔 데 초클로', emoji: '🌽', desc: '옥수수를 갈아 고기 위에 얹어 구운 대표 요리 중 하나예요.' },
      { name: '엠파나다', emoji: '🥟', desc: '고기나 치즈를 넣어 오븐에 구운 반달 모양 빵이에요.' }
    ],
    prides: [
      { name: '모아이 석상', emoji: '🗿', desc: '이스터섬에 세워진, 사람 얼굴을 닮은 거대한 돌 조각상이에요.' },
      { name: '아타카마 사막', emoji: '🏜️', desc: '비가 거의 내리지 않아 밤하늘의 별이 유난히 잘 보이는 사막이에요.' }
    ],
    cultures: [
      { name: '쿠에카', emoji: '💃', desc: '손수건을 흔들며 짝을 지어 추는 칠레의 전통 춤이에요.' },
      { name: '판초', emoji: '🧥', desc: '가운데 구멍으로 머리를 넣어 어깨에 둘러 입는 전통 겉옷 중 하나예요.' }
    ]
  },

...
exports.handler = async (event) => {
  ...
  if (!apiKey) {
    return { statusCode: 503, body: JSON.stringify({ error: '서버 환경변수(OPENAI_API_KEY)가 설정되지 않았습니다.' }) };
  }
  ...
  try {
    ...
    return { statusCode: 400, body: JSON.stringify({ error: '입력값(text)이 올바르지 않습니다.' }) };
  }
  ...
  try {
    ...
    if (!res.ok) {
      ...
      return { statusCode: res.status, body: JSON.stringify({ error: '프롬프트 생성에 실패했습니다.' }) };
    }

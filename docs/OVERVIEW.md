...
| `dictation` | 받아쓰기 도우미 | 받아쓰기 급수표 생성·인쇄, 발음 표기 불러주기, 오류 분석 |
| `kids-dic` | 어린이 쉬운 사전 | 국립국어원 기초사전 기반 초등 눈높이 낱말 풀이 및 사전 (모달·외부 링크) |
...
- **텍스트 전처리 옵션**: 마크다운 기호 무시, 괄호 안 내용 무시, `(pause)`·`/`·줄바꿈을 띄어읽기로 처리(문단 구분은 길게, 단순 줄바꿈은 중간, `/`는 짧게 — 3단계 gap).
- **일시정지/재개**: `speechSynthesis.pause()/resume()`는 브라우저·로컬 음성 조합에 따라 재개가 아예 안 되는 경우가 있어(크로미움 known issue) 사용하지 않음. 대신 문장(`.!?`) 단위로 잘게 나눈 세그먼트를 기억해뒀다가, 일시정지 시 `cancel()` 하고 재개 시 같은 문장부터 다시 `speak()` — 문단 전체가 아니라 현재 문장만 재생되도록 세그먼트를 세분화. 상태막대에 경과 재생 시간(0.25초 간격 갱신)도 표시.
- **읽는 구간 하이라이트**: 세그먼트 시작 시 텍스트창에 포커스 후 해당 구간을 `setSelectionRange`로 선택, `::selection` 커스텀 색상 적용.
- **MP3 다운로드**: Web Speech API는 오디오 스트림을 코드로 얻을 방법이 없어(Web Audio 그래프에 연결되지 않음), `getDisplayMedia({systemAudio:'include'})`로 **시스템 오디오**를 캡처(탭 오디오 공유로는 로컬 SAPI 음성이 안 잡힘 — OS 믹서로 직접 출력되기 때문). 캡처한 webm을 디코딩해 `@breezystack/lamejs`(esm.sh CDN)로 브라우저에서 직접 MP3 인코딩, 서버 전송 없이 다운로드.
- 좌측 사이드바(재생·텍스트처리·다운로드 카드) + 우측 대형 텍스트 입력의 `grid-maker`식 2열 레이아웃. 음성·속도·음높이·체크박스 설정은 `localStorage`에 저장.

...
export async function onRequest(ctx) {
  ...
  try {
    if (action === 'coach') {
      ...
      const systemPrompt = [
        SAFETY_RULE,
        '아이가 고른 물건과 발명 마법을 보고, 아이가 스스로 아이디어를 구체화하도록 짧은 질문 1개를 던져.',
        '질문은 한 문장, 20자 안팎으로 짧게. 그리고 아이가 고를 수 있는 선택지 3개도 함께 만들어(각 10자 이내, 서로 다른 방향).',
    ...
    if (action === 'image') {
      ...
      const prompt =
        '어린이가 크레파스로 그린 그림 스타일. 단순하고 귀여운 형태, 밝고 따뜻한 색, 깨끗한 흰 배경, 텍스트 없음. ' +
